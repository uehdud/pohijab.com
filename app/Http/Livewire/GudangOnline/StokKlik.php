...
class StokKlik extends Component
{
    use WithPagination;
    public $search;
    ...
    {
        $cekstok = ResumeStokOnline::with('detail.kategori', 'detailukuran')
            ->where('kode_merk', 'K')
        ...
        $fotostokpixel = ResumeStokOnline::with('detail.kategori', 'detailukuran', 'foto')
            ...
            ->where('kode_barang', 'like', '%' . $this->search . '%')
            ->paginate(8); */
        $datastokklik = DB::table('resume_stok_onlines')
            ...
            ->where('produk_produksis.kode_merk', 'K')
            ->where('jumlah_stok_online', '>', 0)
 ...
 */
        ...
        /*  $fotostokpixel = DB::table('resume_stok_onlines')
            ...
            ->where('resume_stok_onlines.kode_merk', 'K')
            ->where('jumlah_stok_online', '>', 0)
            ->where('resume_stok_onlines.kode_barang', 'like', '%' . $this->search . '%')
            ->select(
                'image_comp',
        ...
        $fotostokpixel = ResumeStokOnline::with('foto')
            ->where('resume_stok_onlines.kode_barang', 'like', '%' . $this->search . '%')
            ->where('resume_stok_onlines.kode_merk', 'K')
            ->orderBy('lokasi', 'asc')
            ->paginate(10);
        return view('livewire.gudang-online.stok-klik', ['stokpixel' => $datastokklik, 'listfotopo' => $fotostokpixel]);
    }
}
